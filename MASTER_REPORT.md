...
## עדכון 15/03/2026 — שלב 6ב: ניתוח תחומי מומחיות

### מה נוסף

**קובץ חדש: `expert_domain_analyzer.py`**

מנגנון שמנתח את תחומי המומחיות של כל מומחה לפי היסטוריית הפוזיציות שלו ב-Polymarket.

**זרימת עבודה:**
1. שליפת עד 200 פוזיציות (פתוחות + סגורות) מ-`data-api.polymarket.com/positions`
2. סיווג כל שוק לקטגוריה לפי מילות מפתח בשם השוק:
   - ספורט | פוליטיקה | גיאופוליטיקה | קריפטו | כלכלה | טכנולוגיה | בידור
3. קביעת תוצאה לכל פוזיציה: WIN (curPrice ≥ 0.99) / LOSS (curPrice ≤ 0.02) / OPEN
4. חישוב אחוז הצלחה לכל קטגוריה
5. Cache של שעה אחת למניעת קריאות API מיותרות

**שלב חדש ב-Pipeline: שלב 6ב [DOMAIN]**
- לא חוסם — רק מוסיף מידע
- מוסיף שורה לפרופיל Pipeline ולהתראת טלגרם
- מציג: תחום השוק הנוכחי + אחוז הצלחה המומחה בתחום זה + תחום החוזקה שלו

**דוגמה לפלט בהתראה:**
```
🏆 ביצועי המומחה בפוליטיקה: 100% הצלחה (7/7 עסקאות) — תחום חוזקה!
🥇 תחום החוזקה שלו: גיאופוליטיקה (100% הצלחה)
```

### ממצאים מניתוח GCottrell93
| תחום | עסקאות | הצלחה |
|---|---|---|
| פוליטיקה | 10 | 100% |
| גיאופוליטיקה | 5 | 100% |
| קריפטו | 1 | 100% (נתונים חלקיים) |
...
**זרימת עבודה:**
1. שליפת עד 200 פוזיציות (פתוחות + סגורות) מ-`data-api.polymarket.com/positions`
2. סיווג כל שוק לקטגוריה לפי מילות מפתח בשם השוק: ספורט | פוליטיקה | גיאופוליטיקה | קריפטו | כלכלה | טכנולוגיה | בידור
3. קביעת תוצאה לכל פוזיציה: WIN (curPrice >= 0.99) / LOSS (curPrice <= 0.02) / OPEN
4. חישוב אחוז הצלחה לכל קטגוריה
5. Cache של שעה אחת למניעת קריאות API מיותרות
...
### קוד מקור: expert_domain_analyzer.py

ראה קובץ `expert_domain_analyzer.py` בתיקיית הפרויקט לקוד המלא.


---

## עדכון 15 במרץ 2026 — מערכת Value Zone Analysis (שלב 6ג)

### סקירה כללית

מערכת Value Zone Analysis היא שכבת ניתוח חדשה המשלבת שני מקורות נתונים היסטוריים כדי לספק הערכת "ערך" לכל עסקה לפני ביצועה. המערכת עונה על השאלה: **"האם כדאי לקחת את הסיכון הזה בתחום הזה ובמחיר הזה?"**

### ארכיטקטורה: שתי שכבות

#### שכבה 1 — מגמות שוק כלליות (value_zones_global.json)

**מקור:** Polymarket Gamma API — כל השווקים הסגורים
**היקף:** 2,520 שווקים היסטוריים
**מה נמדד:** אחוז YES מנצח לפי תחום

| תחום | YES מנצח | שווקים | אמינות |
|---|---|---|---|
| כלכלה | 51.4% | 37 | ⚠️ חלקי |
| קריפטו | 47.4% | 997 | ✅ אמין |
| אחר | 41.1% | 1,029 | ✅ אמין |
| פוליטיקה | 35.4% | 82 | ⚠️ חלקי |
| ספורט | 35.1% | 208 | ✅ אמין |
| טכנולוגיה | 25.4% | 67 | ⚠️ חלקי |
| גיאופוליטיקה | 21.4% | 42 | ⚠️ חלקי |
| בידור | 8.6% | 58 | ⚠️ חלקי |

**תובנה:** YES מנצח רק ב-35% בספורט ובפוליטיקה — השוק מתמחר YES גבוה מדי. כניסה במחיר נמוך (0.20-0.40) בתחומים אלה מייצגת ערך.

#### שכבה 2 — ביצועי המומחים והלווייתנים שלנו (value_zones_experts.json)

**מקור:** data-api.polymarket.com/positions — כל 20 הארנקים
**היקף:** 20 ארנקים (15 מומחים + 5 לווייתנים) | 4,000+ עסקאות היסטוריות
**מה נמדד:** אחוז הצלחה לפי תחום ולפי קטגוריית מחיר

| תחום | הצלחה | עסקאות | אמינות |
|---|---|---|---|
| גיאופוליטיקה | 97.3% | 111 | ✅ אמין |
| פוליטיקה | 93.8% | 32 | ✅ אמין |
| קריפטו | 93.5% | 31 | ✅ אמין |
| ספורט | 89.1% | 1,159 | ✅ אמין |
| אחר | 86.8% | 2,206 | ✅ אמין |
| כלכלה | 82.4% | 17 | ⚠️ חלקי |

**ממצא מרכזי:** המומחים שלנו מכים את השוק בפער עצום:
- ספורט: שוק 35% → מומחים 89%
- פוליטיקה: שוק 35% → מומחים 94%
- גיאופוליטיקה: שוק 21% → מומחים 97%

### מנוע הניקוד המשולב (market_value_zones.py)

**משקלים:**
- שכבה 2 עם נתוני מחיר (אמין): 70% | שכבה 1: 30%
- שכבה 2 עם נתוני מחיר (לא אמין): 60% | שכבה 1: 40%
- שכבה 2 ללא נתוני מחיר (אמין): 60% | שכבה 1: 40%
- שכבה 2 ללא נתוני מחיר (לא אמין): 50% | שכבה 1: 50%
- רק שכבה 1: 100%

**קטגוריות מחיר (Price Bins):**
- 0.00-0.20 | 0.20-0.35 | 0.35-0.50 | 0.50-0.65 | 0.65-0.75 | 0.75+
...
### שלב 6ג ב-Pipeline (trade_pipeline.py)
...
- **לא חוסם** — מוסיף מידע בלבד
- מוסיף שורה לפרופיל Pipeline ולהתראת טלגרם
- שומר: `signal._value_zone_line`, `signal._value_zone_score`, `signal._value_zone_domain`

### פלט בהתראת טלגרם

```
📊 Value Zone: ספורט | מחיר 0.35-0.50
🌍 שוק כללי: 35.1% YES מנצח (208 שווקים) ✅
🏆 מומחים שלנו [0.35-0.50]: 89.9% הצלחה (187/208) ✅
⚡ ניקוד משולב: 73.5% | 🟢 חזק | ביטחון: גבוהה
```
...
| קובץ | תיאור |
|---|---|
| `market_value_zones.py` | מנוע Value Zone המשולב — טעינה, ניתוח, פורמט |
| `value_zones_global.json` | שכבה 1 — מגמות שוק כלליות (2,520 שווקים) |
| `value_zones_experts.json` | שכבה 2 — ביצועי מומחים/לווייתנים (20 ארנקים) |
| `build_value_zones.py` | סקריפט לבניית שכבה 1 מחדש |
| `build_expert_zones.py` | סקריפט לבניית שכבה 2 מחדש |

### עדכון Value Zones

כדי לעדכן את הנתונים ההיסטוריים:
```bash
# עדכון שכבה 1 (שווקים כלליים)
python3.11 build_value_zones.py

# עדכון שכבה 2 (מומחים ולווייתנים)
python3.11 build_expert_zones.py
```

מומלץ לרוץ פעם בשבוע כדי לשמור על נתונים עדכניים.
